...
export default class BillingController extends Controller {
  ...
  load = async function (ctx) {
    ...
    try {
      ...
      if (ctx.invoice.restricted_countries && (ctx.invoice.restricted_countries.length > 0)){
        ctx.countries = ctx.countries.filter(function(c){ return !ctx.invoice.restricted_countries.includes(c.code); });
      }
      ctx.loading = false;
      ...
      if (ctx.invoice.billing) {
        ctx.billing = ctx.invoice.billing;
        if (!ctx.billing.email) {
        ...
        console.log('Trying to transitionTo email');
        ctx.router.transitionTo('email', ctx.invoiceID);
      }
    } catch (e) {
      console.log(e.stack);
  ...
  continue = async function(ctx){
    ...
    if (!ctx.billing.country){ ctx.focus('billingCountry'); return }

    if (ctx.loading) return;
    ctx.loading = true;
    try{
      var res = await ctx.api.updateBilling(ctx.invoiceID, ctx.billing);
      if (res.error){
        ctx.error = res.error;
      } else {
        ctx.router.transitionTo('pay', ctx.invoiceID);
  ...
  actions = {
    ...
    billingFirstNameInputFocusOut: function(){ this.billingFirstNameInputFocused = false; },
    billingAddressInputFocusIn: function(){ this.billingAddressInputFocused = true; },
    billingAddressInputFocusOut: function(){ this.billingAddressInputFocused = false; },
    billingCityInputFocusIn: function(){ this.billingCityInputFocused = true; },
    billingCityInputFocusOut: function(){ this.billingCityInputFocused = false; },
    billingPostalInputFocusIn: function(){ this.billingPostalInputFocused = true; },
    billingPostalInputFocusOut: function(){ this.billingPostalInputFocused = false; },
    billingCountryInputFocusIn: function(){ this.billingCountryInputFocused = true; },
    billingCountryInputFocusOut: function(){ this.billingCountryInputFocused = false; },
    selectCountry: function(selected){ this.billing.country = selected; },
  };
